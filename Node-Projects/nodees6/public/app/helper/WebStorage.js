class WebStorage {
    ...
    truncate(table_name) {
        //delte obect
        if (this.tableExists(table_name) && this.rowCount(table_name) > 0) {
            this.db.tables[table_name].auto_increment = 1;
            console.log("table '" + table_name + "' remove complet!");
        } else {
            console.log("table '" + table_name + "' not exists!");
        }
    }
    ...
    selectByKV(items, queryObj, limit) {
        let results = [];
                ...
                if (k == key[0]) { //first key
                    ...
                    for (let c = 0; c < vals.length; c++) {
                        if (record[k] == vals[c].trim()) {//compare values
                            if (record != undefined) {
                                status = true;
                                results.push(record); //keep record
                            }
                            if ((!this.isBlank(limit)) && results.length == limit) {
                                return results;
                            }
                        }
                    }
